import { initTD } from './componentsCaidaTension/TDManager.js';
import { initTG } from './componentsCaidaTension/TGManager.js';
import { initSELECCION } from './componentsCaidaTension/SelectionManager.js';

// Función para registrar el componente Alpine
function registerCaidaTensionSystemComponent() {
    ...
    if (window.Alpine._caidatensionSystemRegistered) {
        console.log('Componente aguaSystem ya registrado');
        return;
    }

    console.log('Registrando componente caidatensionSystem...');

    window.Alpine.data('caidatensionSystem', () => ({
        ...
        tabs: [
            { id: 'td', name: 'TABLEROS DE DISTRIBUCION', icon: 'fa-chart-bar' },
            { id: 'tg', name: 'TABLERO GENERAL', icon: 'fa-cube' },
            { id: 'selection', name: 'SELECCION DE GRUPO', icon: 'fa-water' },
        ],
        ...
        loadTabContent() {
            const modules = [
                { func: initTD, name: 'td', containerId: 'td-content' },
                { func: initTG, name: 'tg', containerId: 'tg-content' },
                { func: initSELECCION, name: 'selection', containerId: 'selection-content' },
            ];
    ...
    window.Alpine._caidatensionSystemRegistered = true;
    console.log('Componente caidatensionSystem registrado correctamente');
}
...
function shouldRegisterComponent() {
    ...
    const isWaterPage = window.location.pathname.includes('agua') ||
        document.querySelector('[x-data="caidatensionSystem"]') !== null;
...
if (shouldRegisterComponent()) {
    ...
    if (typeof window.Alpine !== 'undefined') {
        registerCaidaTensionSystemComponent();
    } else {
        ...
        setTimeout(() => {
            if (typeof window.Alpine !== 'undefined' && !window.Alpine._caidatensionSystemRegistered) {
                registerCaidaTensionSystemComponent();
            }
        }, 100);
    }
