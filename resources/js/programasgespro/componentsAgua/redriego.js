...
export function initRedriegosModule() {
    const redRiego = document.getElementById('red-griego-content');
    if (!redRiego) {
        console.error('Contenedor Red de Riego no encontrado');
        return;
    }

    redRiego.innerHTML = `
        <div x-data="redRiego" class="w-full p-4">
            <!-- Header Principal -->
                ...
                <div class="max-w-7xl mx-auto px-6 py-4">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-4">
                            ...
                            <div>
                                <h1 class="text-2xl font-bold text-slate-800">7. CALCULO DE LA RED DE RIEGO</h1>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-3 p-4 bg-white rounded-xl shadow">
                        <div class="space-y-1">
                            ...
                            <div class="flex items-center">
                                <input type="number" x-model.number="config.npisoterminado" step="0.001" @input="caudalImpulsion.updateCalculations()"
                        ...
                        <div class="space-y-1">
                            ...
                            <div class="flex items-center">
                                <input type="number" x-model.number="config.altasumfondotanqueelevado" step="0.001" @input="caudalImpulsion.updateCalculations()"
                                    class="w-full px-3 py-2 bg-yellow-100 border-2 border-yellow-300 text-gray-700 font-semibold rounded-l-md focus:outline-none">
                                <span class="px-3 py-2 bg-gray-200 text-sm text-gray-700 rounded-r-md border-t border-b border-r border-gray-300">m</span>
                            </div>
                        </div>
    ...
    Alpine.data('redRiego', () => ({
                ...
                Object.keys(this.tables).forEach(grado => {
                    if (this.grades[grado] && this.tables[grado].modules.length === 0) {
                        ...
                        this.$nextTick(() => {
                            ...
                            this.sendredInterioresUpdate(); // Enviar datos tras inicializar la tabla
                        });
                    }
                });
        ...
        configurarEventos() {
            // Evento existente para maxima-demanda-simultanea
            ...
            // NUEVO EVENTO: red-alimentacion-updated
            document.addEventListener('red-alimentacion-updated', (event) => {
                ...
                // Almacenar los datos de red de alimentación
                this.redAlimentacionData = JSON.parse(JSON.stringify(event.detail.tablas || {}));

                // Sincronizar todos los módulos que tengan filas estáticas con puntos definidos
                this.sincronizarConRedAlimentacion();
            });
        },

        // Función para sincronizar con datos de red de alimentación
        sincronizarConRedAlimentacion() {
            if (!this.redAlimentacionData || Object.keys(this.redAlimentacionData).length === 0) {
                console.log("No hay datos de red de alimentación para sincronizar");
                return;
            }

            // Recorrer todos los grados activos
            Object.keys(this.grades).forEach(grado => {
                if (!this.grades[grado]) return;

                // Recorrer todos los módulos del grado (excepto el primero si quieres mantener esa lógica)
                this.tables[grado]?.modules?.forEach((modulo, moduloIndex) => {
                    // Buscar filas estáticas en el módulo
                    if (Array.isArray(modulo.data)) {
                        modulo.data.forEach(fila => {
                            if (fila.isStatic && fila.punto) {
                                // Buscar el valor hpiez en los datos de red de alimentación
                                const hpiezEncontrado = this.buscarHpiezEnRedAlimentacion(grado, fila.punto);
            ...
            // Enviar actualización después de sincronizar
            this.sendredInterioresUpdate();
        },
        // Función para buscar hpiez en los datos de red de alimentación
        buscarHpiezEnRedAlimentacion(grado, puntoABuscar) {
            if (!puntoABuscar || !this.redAlimentacionData[grado]) {
                return null;
            }

            console.log(`Buscando punto "${puntoABuscar}" en grado "${grado}"`);
            console.log("Datos disponibles:", this.redAlimentacionData[grado]);

            // Recorrer todas las tablas del grado en red de alimentación
            for (let tabla of this.redAlimentacionData[grado]) {
                if (!Array.isArray(tabla.data)) continue;

                // Buscar en los datos de la tabla
        ...
        // Función modificada para manejar cambio de punto
        manejarCambioPunto(grado, moduloId, nuevoPunto, filaId) {
            ...
            if (filaEstatica && nuevoPunto) {
                // Buscar el valor hpiez en los datos de red de alimentación
                const hpiezEncontrado = this.buscarHpiezEnRedAlimentacion(grado, nuevoPunto);
        ...
        // Función para sincronizar un punto específico en todos los módulos
        sincronizarPuntoEnTodosModulos(grado, punto, nuevoHpiez) {
            if (!punto || nuevoHpiez === undefined || nuevoHpiez === null) return;

            const hpiezValue = parseFloat(nuevoHpiez) || 0;

            // Recorrer todos los módulos del grado
            this.tables[grado]?.modules?.forEach((modulo) => {
                // Buscar fila estática en los datos del módulo
                const filaEstatica = modulo.data.find(fila => fila.isStatic);
                if (!filaEstatica) return;
        ...
        obtenerInfoDebug() {
            return {
                redAlimentacionData: this.redAlimentacionData,
                grades: this.grades,
                tablesInfo: Object.keys(this.tables).map(grado => ({
                    grado,
                    modulosCount: this.tables[grado].modules.length,
                    modulos: this.tables[grado].modules.map(m => ({
                        id: m.id,
                        nombre: m.nombre,
        ...
        crearModulo(grado) {
            ...
            const nombre = `RED DE RIEGO - ${grado.toUpperCase()}`;
            ...
            const columns = [
                // Columnas principales
                {
                    ...
                    cellEdited: function (cell) {
                        ...
                        if (editable && typeof this.sendredInterioresUpdate === 'function') {
                            this.sendredInterioresUpdate();
                        }
                    }.bind(this)
                },
                ...
                // Columna Punto MEJORADA
                {
                    ...
                    cellEdited: editable ? function (cell) {
                        ...
                        // Usar la nueva función para manejar el cambio con red de alimentación
                        this.manejarCambioPunto(grado, moduloId, nuevoPunto, rowData.id);

                        if (typeof this.sendredInterioresUpdate === 'function') {
                            this.sendredInterioresUpdate();
                        }
                    }.bind(this) : undefined
                },
                {
                    ...
                    headerVertical: true,
                    cellEdited: editable ? () => { this.sendredInterioresUpdate(); } : undefined
                },
...
//document.addEventListener('DOMContentLoaded', initRedriegosModule);