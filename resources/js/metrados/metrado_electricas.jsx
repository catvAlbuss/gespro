...
const UNIT_EDITABLE_COLS = {
    ...
    'm²v₁': [COL.ELEM_L, COL.L, COL.ANC, COL.ALT],
...
const API = {
    async cargarDatos() {
        const idmetrado = document.getElementById('id_melectricas')?.value;
        if (!idmetrado) return null;

        try {
            const response = await fetch(`/metrados/electricas/${idmetrado}`, {
                headers: {
    ...
    async guardarDatos(rows, resumenData) {
        ...
        try {
            const response = await fetch('/metrados/electricas/actualizar', {
                method: 'POST',
                headers: {
...
export default function MetradoOptimizado() {
    ...
    const updateCellImmediate = useCallback((rowIndex, col, value) => {
        ...
        const numericCols = [COL.ELEM_L, COL.L, COL.ANC, COL.ALT, COL.NVECES, COL.VOL];
    ...
    const exportToExcel = useCallback(async () => {
        ...
        const options = {
            ...
            especialidad: 'ELECTRICAS', // o podrías usar dinámico si lo tienes en tu JSON
            logo1: {
                data: logo1Buffer,
                extension: logo1Ext
            },
...
const container = document.getElementById('metradoElectricas');
if (container) {
