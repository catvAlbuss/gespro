<x-app-layout>
    <x-slot name="header">
        ...
        <div class="flex flex-col sm:flex-row flex-wrap items-center justify-start mt-4 space-x-2 py-2">
            ...
            <button id="abrir_equipamiento" type="button"
                class="relative py-1 px-2 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50 focus:outline-none focus:bg-gray-50 disabled:opacity-50 mb-2 sm:mb-0">Equipos
                Entregado
    ...
    <script>
        ...
        function calcularEntrada(horaLima) {
            const hora = horaLima.toLocaleTimeString('es-PE', {
                hour12: false
            });
        
            let [horas, minutos] = hora.split(':').map(Number);
            let horaActual = horas + minutos / 60;
        
            if (horaActual >= 8 && horaActual < 8.5) {
                return 'Puntual';
            } else if (horaActual >= 8.5 && horaActual < 13) {
                return 'Tarde';
            } else if (horaActual >= 15 && horaActual < 15.33) {
                return 'Puntual';
            } else if (horaActual >= 15.33 && horaActual <= 20) {
                return 'Tarde';
            } else {
