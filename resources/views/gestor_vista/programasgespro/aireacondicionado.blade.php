<x-app-layout>
    ...
    <!-- Contenedor Vue -->
    <div id="ac-calculator-app" data-module="programasgespro/aireacondicionado" class="container mx-auto px-4 py-6">
        <!-- Header -->
        <div class="bg-white rounded-lg shadow-lg p-6 mb-6">
            <div class="flex items-center justify-between mb-4">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-gray-800">Calculadora AC Pro</h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select v-model="climateType" @change="updateClimateSettings($event)"
                ...
                <div>
                    ...
                    <div class="flex space-x-2">
                        <input type="number" v-model="newSheetsCount" min="1" max="10"
                            class="w-full appearance-none bg-white border-2 border-gray-200 rounded-xl px-4 py-3 pr-10 text-gray-700 focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100 transition-all duration-200 font-medium">
        ...
        <transition name="fade">
            <div v-show="error" class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4">
                <span v-text="error"></span>
            </div>
        </transition>
        <!-- Success Display -->
        <transition name="fade">
            <div v-show="success" class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-4">
                    ...
                    <div class="p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div class="bg-blue-50 p-4 rounded-lg">
                                ...
                                <p class="text-2xl font-bold text-blue-600" v-text="getTotalThermalLoad() + ' BTU'"></p>
                            </div>
                            <div class="bg-green-50 p-4 rounded-lg">
                                <h4 class="font-semibold text-green-800">Total AC Requerido</h4>
                                <p class="text-2xl font-bold text-green-600" v-text="getTotalACRequired() + ' BTU'"></p>
                            </div>
                            <div class="bg-yellow-50 p-4 rounded-lg">
                                <h4 class="font-semibold text-yellow-800">Hojas Activas</h4>
                                <p class="text-2xl font-bold text-yellow-600" v-text="sheets.length"></p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <template v-for="(sheet, index) in sheets" :key="sheet.id">
                <div class="bg-white rounded-lg shadow-lg overflow-hidden">
                    ...
                    <div class="bg-blue-600 text-white p-4 flex justify-between items-center">
                        <div>
                            <h3 class="font-semibold" v-text="sheet.name"></h3>
                            <p class="text-sm opacity-90" v-text="'Área: ' + sheet.area + ' m²'"></p>
                        </div>
                        <div class="flex space-x-2">
                    ...
                    <div class="p-4">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" v-model="sheet.area" @input="updateSheetCalculations(index)"
                        ...
                        <div class="mb-6">
                            ...
                            <div class="space-y-2">
                                <template v-for="(item, itemIndex) in sheet.thermalLoad" :key="item.id">
                                    <div class="flex items-center space-x-2 bg-gray-50 p-2 rounded">
                                        <input type="text" v-model="item.description" placeholder="Descripción"
                                            class="flex-1 px-2 py-1 border border-gray-300 rounded text-sm">
                                        <input type="number" v-model="item.btu"
                                        ...
                                        <span class="w-20 text-sm font-medium"
                                            v-text="(item.btu * item.quantity) || 0"></span>
                        ...
                        <div class="mb-6">
                            <div class="flex justify-between items-center mb-3">
                                <h4 class="font-semibold text-gray-800">Tipos de AC</h4>
                                <button @click="addACRow(index)"
                                    class="px-3 py-1 bg-green-600 text-white text-sm rounded hover:bg-green-700 transition-colors">
                                    + Agregar
                                </button>
                            </div>
                            <div class="space-y-2">
                                <template v-for="(ac, acIndex) in sheet.acTypes" :key="ac.id">
                                    <div class="flex items-center space-x-2 bg-blue-50 p-2 rounded">
                                        <input type="number" v-model="ac.quantity"
                                            @input="updateSheetCalculations(index)" placeholder="Cant."
                                            class="w-16 px-2 py-1 border border-gray-300 rounded text-sm">
                                        <select v-model="ac.btu" @change="updateSheetCalculations(index)"
                                        ...
                                        <button @click="removeACRow(index, acIndex)"
                                            class="p-1 text-red-600 hover:bg-red-100 rounded">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div class="border-t pt-4 space-y-2">
                            <div class="flex justify-between text-sm">
                                ...
                                <span v-text="getAreaLoad(sheet) + ' BTU'"></span>
                            </div>
                            <div class="flex justify-between text-sm">
                                <span>Carga por equipos:</span>
                                <span v-text="getThermalLoadTotal(sheet) + ' BTU'"></span>
                            </div>
                            <div class="flex justify-between text-sm">
                                <span>Capacidad total AC:</span>
                                <span v-text="getACTotal(sheet) + ' BTU'"></span>
                            </div>
                            <div class="flex justify-between font-semibold text-lg border-t pt-2"
                                :class="getSheetTotal(sheet) >= 0 ? 'text-green-600' : 'text-red-600'">
                                <span>Balance:</span>
                                <span v-text="getSheetTotal(sheet) + ' BTU'"></span>
                            </div>
                            <div class="flex justify-between text-sm">
                                <span>Cantidad de Aire ACONDICIONADO:</span>
                                <span v-text="getTotalUnits(sheet)"></span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="mt-6 text-center" v-show="sheets.length === 0">
