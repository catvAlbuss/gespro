<x-app-layout>
                ...
                <section class="bg-white dark:bg-gray-900 text-gray-950 dark:text-white">
                    <div class="py-8 lg:py-16 px-4 mx-auto max-w-screen-md">
                        <div class="grid gap-4 sm:grid-cols-3 sm:gap-6">
                            ...
                            <button type="button" id="calculartanque"
    ...
    <script>
        ...
        $('#calculartanque').on("click", function() {
            ...
            const cisternaElevadoVal = ((1.25 / 3) * dotacionFinal);
            crearpdfcisterna(area, npisos, valdotacion, dotacionFinal, cisternaElevadoVal)
        });

        const tanquescisternas = [{
                nombre: 'Tanque de Agua Negro 3 capas',
                capacidad: 1100,
                imagen: "{{ asset('storage/eternit/tanque-negro-1100-lts-foto-interna4.png') }}" // Imagen obtenida desde Laravel
            },
            {
                nombre: 'Tanque de Agua Azul 3 capas',
                capacidad: 1100,
                imagen: "{{ asset('storage/eternit/tanque-azul_1100-lts-foto-interna7.png') }}" // Imagen obtenida desde Laravel
            },
            {
                nombre: 'Tanque de Agua Arena 4 capas',
                capacidad: 1100,
                imagen: "{{ asset('storage/eternit/tanque-arena-4-capas_1100-lts-foto-interna-5.png') }}" // Imagen obtenida desde Laravel
            },
            {
                nombre: 'Tanque Industrial Reforzado Negro',
                capacidad: 25000,
                imagen: "{{ asset('storage/eternit/tanque-reforzado-25k-ltsfoto-interna.png') }}" // Imagen obtenida desde Laravel
            },
            {
                nombre: 'Tanque Industrial Extrareforzado Negro',
                capacidad: 10000,
                imagen: "{{ asset('storage/eternit/tanque-extrareforzado - 10000lts-foto-interna-3.png') }}" // Imagen obtenida desde Laravel
            },
            {
                nombre: 'Tanque Industrial Estándar Negro',
                capacidad: 25000,
                imagen: "{{ asset('storage/eternit/tanque-estandar-5k-lts_foto-interna1.png') }}" // Imagen obtenida desde Laravel
            }
        ];

        // Función para validar el tanque adecuado
        function validarcisterna(cisternaElevadoVal) {
            const rangoMinimo = cisternaElevadoVal - 500;
            const rangoMaximo = cisternaElevadoVal + 500;
        ...
        function crearpdfcisterna(area, npisos, valdotacion, dotacionFinal, cisternaElevadoVal) {
            ...
            cisternapdf.text("Tanque Elevado", 90, 20);
            ...
            cisternapdf.text("Formula para el Volumen = 1.25/3 DOTACION", 20, 95);
            cisternapdf.text("Volumen = " + cisternaElevadoVal + " Litros", 40, 105);
            cisternapdf.text("Tanque: " + resultado.nombre, 40, 115);
            cisternapdf.text("Capacidad: " + String(resultado.capacidad) + " Litros", 120, 115);
            // Agregar la imagen del tanque al PDF
            cisternapdf.addImage(resultado.imagen, 'PNG', 20, 120, 180, 160);
