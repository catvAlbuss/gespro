                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    {{-- <div class="grid gap-4 sm:grid-cols-3 sm:gap-6">
                        <div class="col-span-3 text-center text-2xl">
                            <h2 for="rmis">RESUMEN DE METRADOS INSTALACIONES SANITARIAS</h2>
                        </div>
                        <div class="col-span-3 text-center">
                            <x-input-label for="nombre_proyecto" :value="__('Nombre_proyecto')" />
                            <x-text-input type="text" name="nombre_proyecto" id="nombre_proyecto"
                                value="MEJORAMIENTO DE LOS SERVICIOS DE EDUCACION INICIAL Y PRIMARIA DE LA I.E.I.P N°64193 CONTAMANA DISTRITO DE CONTAMANA-PROVINCIA DE UCAYALI-DEPARTAMENTO DE LORETO"
                                class="block mt-1 w-full text-center" required />
                        </div>
                        <div>
                            <x-input-label for="entidadm" :value="__('UEI')" />
                            <x-text-input type="text" name="entidadm" id="entidadm"
                                value="MUNICIPALIDAD PROVINCIAL DE UCAYALI" class="block mt-1 w-full text-center"
                                required />
                        </div>
                        <div>
                            <x-input-label for="Fecha" :value="__('Fecha')" />
                            <x-text-input type="date" name="Fecha" id="Fecha" value="2024-10-01"
                                class="block mt-1 w-full text-center" required />
                        </div>
                        <div>
                            <x-input-label for="especialidad" :value="__('Especialidad')" />
                            <x-text-input type="text" name="especialidad" id="especialidad"
                                value="INSTALACIONES SANITARIAS-RESUMEN" class="block mt-1 w-full text-center"
                                required />
                        </div>
                        <div>
                            <x-input-label for="codejecucion" :value="__('CUI')" />
                            <x-text-input type="number" name="codejecucion" id="codejecucion" value="2484985"
                                class="block mt-1 w-full text-center" required />
                        </div>
                        <div>
                            <x-input-label for="codejecucion" :value="__('CÓDIGO MODULAR')" />
                            <x-text-input type="text" name="codejecucion" id="codejecucion" value="0304881"
                                class="block mt-1 w-full text-center" required />
                        </div>
                        <div>
                            <x-input-label for="codejecucion" :value="__('CÓDIGO LOCAL')" />
                            <x-text-input type="text" name="codejecucion" id="codejecucion" value="391051"
                                class="block mt-1 w-full text-center" required />
                        </div>
                        <div>
                            <x-input-label for="codejecucion" :value="__('Localidad')" />
                            <x-text-input type="text" name="codejecucion" id="codejecucion"
                                value="Contamana-Contamana-Ucayali-Lorato" class="block mt-1 w-full text-center"
                                required />
                        </div>
                        <div>
                            <x-input-label for="modulos" :value="__('Cantidad de Modulos')" />
                            <x-text-input type="number" name="modulos" id="modulos" value="1"
                                class="block mt-1 w-full text-center" required />
                        </div>
                        <div class="flex items-center justify-end mt-4">
                            <x-blue-button class="ml-4">
                                {{ 'Generar Metrados Sanitarias' }}
                            </x-blue-button>
                        </div>
                    </div>

                    <a href="{{ route('gestormetradoSanitariashow', ['empresaId' => $empresaId]) }}"
                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Abrir</a>
                    <a href="{{ route('gestormetradometradoelectricasshow', ['empresaId' => $empresaId]) }}"
                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Abrir electricas</a>
                    <h3 class="text-lg font-semibold">ID: {{ $empresaId }}</h3> --}}
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead
                                ...
                                <tr>
                                    ...
                                    <th scope="col" class="px-4 py-3">Cantidad Modulo</th>
                            ...
                            <tbody>
                                @foreach ($metradosanitarias as $metrado)
                                    <tr
                                        ...
                                        <td class="px-4 py-2 font-medium text-gray-900  dark:text-white">
                                            {{ $metrado->cantidadModulo }}</td>
                                        <td class="px-4 py-2 font-medium text-gray-900  dark:text-white">
                                            <a href="{{route('metradosanitarias.show', $metrado->idmetradosan)}}" class="btn text-blue-500">Visualizar</a>
                                        </td>
                                        <td class="px-4 py-2 font-medium text-gray-900 dark:text-white">
                                            <a href="{{ route('metradosanitarias.edit', $metrado->idmetradosan) }}" class="btn text-yellow-500">Editar</a>
                                        </td>
                                        <td class="px-4 py-2">
                                            <form
    ...
    <div id="crud-modal" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <!-- Modal body -->
                <form class="p-4 md:p-5" method="POST" action="{{ route('metradosanitarias.store') }}">
                    @csrf
                    <div class="grid gap-4 mb-4 grid-cols-2">
                        ...
                        <div class="col-span-2 sm:col-span-1">
                            <label for="codigo_modular"
                                ...
                                MODULAR</label>
                            <input type="number" name="codigo_modular" id="codigo_modular"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                placeholder="2999" required="">
                        </div>
                        <div class="col-span-2 sm:col-span-1">
                            <label for="codigo_local"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">CÓDIGO
                                LOCAL</label>
                            <input type="number" name="codigo_local" id="codigo_local"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                placeholder="2999" required="">
                        </div>
                        <div class="col-span-2 sm:col-span-1">
                            <label for="cantidadModulo"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Cantidad
                                Modulo</label>
                            <input type="number" name="cantidadModulo" id="cantidadModulo"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                placeholder="1" value="3" required="">
                        </div>
