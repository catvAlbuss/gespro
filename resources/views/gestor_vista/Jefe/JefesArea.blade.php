<x-app-layout>
    <x-slot name="header">
        ...
        @else
            <div class="mt-4">
                <p class="text-red-600">No hay empresa asociada.</p>
            </div>
        @endif

        <div class="flex flex-col sm:flex-row items-center justify-center mt-4 space-x-2">
            <!--<button id="marcar-asistencia" type="button" class="relative py-1 px-2 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50 focus:outline-none focus:bg-gray-50 disabled:opacity-50">
                Marcar Asistencia
            </button>-->
            ...
            <button id="abrir-tareas-trab" type="button" class="relative py-1 px-2 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50 focus:outline-none focus:bg-gray-50 disabled:opacity-50">
                Tareas
                <span class="flex absolute top-0 right-0 -mt-2 -mr-2">
            ...
            <button id="abrir_equipamiento" type="button"
                class="relative py-1 px-2 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50 focus:outline-none focus:bg-gray-50 disabled:opacity-50">Equipos
                Entregado
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            function cargarDatos(empresaId) {
                $.ajax({
                    ...
                    success: function(data) {
                        // Asumiendo que data es un objeto con las propiedades events, usuarios y proyectos
                        const proyectosInput = JSON.stringify(data.proyectos);
                        const trabajadoresInput = JSON.stringify(data.usuarios);
            ...
            function mostrarDatos(proyectosInput, trabajadoresInput, eventData) {
                ...
                const calendars = trabajadores
                    .filter(trabajador => 
                        trabajador.name !== "Administrador" && 
                        !excludedNames.includes(trabajador.name) // Filtra los nombres a excluir
                    )
                    .map(trabajador => {
                        return {
                            id: trabajador.id,
                ...
                const calendar = new EventCalendar("#calendar", {
                    ...
                    //editorShape: editorShape,
                    //tareasInsert: tareasInsert, // Añade el nuevo editorShape aquí
                    locale: {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (!data || data.length === 0) {
                        ...
                        window.tareas = data;
                        // Iniciar el proceso de marcado de asistencia
    ...
    <script>
        async function obtenerAsistencia(empresaId, trabajadorId) {
            try {
                ...
                if (response.ok) {
                    ...
                    console.error('Error al obtener la asistencia:', data.error);
                }
            } catch (error) {
        ...
        function renderizarTabla(data) {
            ...
            data.forEach((item, index) => {
                const totalNoMarcado = parseInt(noMarcado) - (parseInt(item.puntualidad) + parseInt(item.tardanza));
                tableContent += `
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        <td class="px-6 py-4" rowspan="3">${index + 1}</td>
                        <th scope="row" rowspan="3" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            ${item.nombre}
                        </th>
                        <td class="px-6 py-4">Puntualidad</td>
                        <td class="px-6 py-4">${item.puntualidad}</td>
                    </tr>
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        <td class="px-6 py-4">Tardanza</td>
                        <td class="px-6 py-4">${item.tardanza}</td>
                    </tr>
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        <td class="px-6 py-4">Sin Marcar</td>
                        <td class="px-6 py-4">${totalNoMarcado}</td>
                    </tr>
                `;
            });
            document.getElementById('asistenicaPersonal').innerHTML = tableContent;
        }

        // Llamar a obtenerAsistencia con los IDs correspondientes
        document.addEventListener('DOMContentLoaded', () => {
            const empresaId = document.getElementById('empresaId').value;
            const trabajadorId = document.getElementById('trabajador_id').value;
            obtenerAsistencia(empresaId, trabajadorId);
        });
    </script>
