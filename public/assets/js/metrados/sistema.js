...
export default class Sistema {
    ...
    /*updateResumenTable() {
        // Add timer variable at the beginning of the method
        if (!this.updateTimer) {
        ...
        // Modify the click handler for the button
        $('#actualizar_metrados').off('click').on('click', () => {
            // Toggle auto-update state
            this.isAutoUpdateActive = !this.isAutoUpdateActive;
            const button = $('#actualizar_metrados');

            if (this.isAutoUpdateActive) {
                // Start auto-update
                button.text('Detener Auto-actualización'); // Change button text
                button.addClass('active'); // Optional: add visual indicator
                ...
                // Stop auto-update
                button.text('Iniciar Auto-actualización'); // Restore button text
                button.removeClass('active'); // Optional: remove visual indicator
        ...
        const isValidItem = (item) => {
            ...
            // Allow any item length as long as each part is a 2-digit number
            ...
            if (node.item && node.item.trim() !== '') {
                if (!resumenData[key]) {
                    resumenData[key] = {
                        ...
                        unidad: node.unidad || '',
                        // Only set values if they're not empty/zero
        ...
        const processNode = (node, options = {}) => {
            ...
            if (record) {
                ...
                // Process values only if totalnieto is not null/undefined
    ...
    updateResumenTable() {
        ...
        setTimeout(() => {
            ...
            resumenMetradosSanitarias.length = 0;
        ...
        $('#actualizar_metrados').on('click', () => {
            ...
            const datacisterna = this.cisternaTable?.table?.getData();
            const dataexterior = this.exteriorTable?.table?.getData();
            const resumenDataSan = resumenMetradosSanitarias;
            console.log(resumenDataSan);
            // Create a JSON object to send via AJAX
            const dataToSend = {
                modulos: modulosData,
                cisterna: datacisterna,
