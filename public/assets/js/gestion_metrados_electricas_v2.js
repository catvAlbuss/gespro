$(document).ready(function () {
    ...
    //const metradoElectricas = sortTreeData(database.modulos);
    let metradoElectricas = sortTreeData(database.modulos);
    ...
    const listaNormativas = [
        {
            item: "INSTALACIONES ELÉCTRICAS Y MECÁNICAS", children: [
                { item: "CONEXIÓN A LA RED EXTERNA DE MEDIDORES" },
                {
                    item: "SALIDAS PARA ALUMBRADO, TOMACORRIENTES, FUERZA Y SEÑALES DÉBILES", children: [
                        { item: "SALIDA" },
                        { item: "CANALIZACIONES, CONDUCTOS O TUBERÍAS" },
                        { item: "CONDUCTORES Y CABLES DE ENERGÍA EN TUBERÍAS" },
                        { item: "SISTEMAS DE CONDUCTOS" },
                        { item: "INSTALACIONES EXPUESTAS" },
                        { item: "TABLEROS PRINCIPALES" },
                        { item: "TABLERO DE DISTRIBUCIÓN" },
                        { item: "DISPOSITIVOS DE MANIOBRA Y PROTECCIÓN" },
                    ]
                },
                { item: "INSTALACION DE PARARRAYOS" },
                { item: "INSTALACIÓN DEL SISTEMA DE PUESTA A TIERRA" },
                {
                    item: "ARTEFACTOS", children: [
                        { item: "LÁMPARAS" },
                        { item: "REFLECTORES" },
                    ]
                },
                {
                    item: "EQUIPOS ELECTRICOS Y MECANICOS", children: [
                        { item: "BOMBA PARA AGUA" },
                        { item: "BOMBAS PARA DESAGÜE (IGUAL A BOMBAS PARA AGUA)" },
                        { item: "OTRAS BOMBAS" },
                        { item: "GRUPOS ELECTRÓGENOS" },
                        { item: "SISTEMA DE RECIRCULACIÓN" },
                        { item: "ASCENSORES Y MONTACARGAS" },
                        { item: "SISTEMAS DE PARLANTES" },
                        { item: "SISTEMA DE MÚSICA AMBIENTAL" },
                        { item: "SISTEMA DE TRADUCCIÓN SIMULTANEA" },
                        { item: "SISTEMA DE SEGURIDAD" },
                        { item: "PROYECTORES Y PANTALLAS" },
                        { item: "CAMPANAS EXTRACTORAS" },
                        { item: "SISTEMA DE VAPOR" },
                        { item: "SISTEMA DE AIRE COMPRIMIDO" },
                        { item: "SISTEMA DE OXÍGENO" },
                        { item: "SISTEMA DE VENTILACIÓN MECÁNICA" },
                        { item: "SISTEMA DE VACÍO" },
                        { item: "SISTEMA DE AIRE ACONDICIONADO" },
                    ]
                },
            ]
        }
    ...
    var table = new Tabulator("#metrados-table", {
        movableRows: true, //enable user movable rows
        height: "500px",
        data: metradoElectricas,
        layout: "fitColumns",
        dataTree: true,
    ...
    function updateMetrados() {
        const id_metrado_electricas = document.getElementById('id_melectricas').value;
        const datatable = metradoElectricas;
        const dataToSend = {
            id: id_metrado_electricas,
            modulos: datatable,
        };

        $.ajax({
            url: '/update_metrados_electricas',
            method: 'POST',
   ...
   /* $('#actualizar_metrados').on('click', () => {
        ...
        $.ajax({
            url: '/update_metrados_electricas', // Laravel route
            method: 'POST',
    ...
    const resumenmetradoelectricas = {};
    const resumenMetradoElectricasTree = [];
    ...
    const upsertResumenRecord = (key, node) => {
        ...
        if (node.item && node.item.trim() !== '') {
            if (!resumenmetradoelectricas[key]) {
                resumenmetradoelectricas[key] = {
                    id: node.id,
                    item: node.item,
            ...
            return resumenmetradoelectricas[key];
        }
        return null;
    };
    ...
    const processNode = (node, options = {}) => {
        ...
        if (node.item && typeof node.item === 'string' && node.item.trim() !== '') {
            ...
            if (treeParent) {
                ...
                resumenMetradoElectricasTree.push(treeNode);
            }
    ...
    metradoElectricas.forEach(node => {
        processNode(node, { sourceType: 'exterior' });
    });

    // Mostrar el resumen en la tabla
    var table2 = new Tabulator("#metrados-resumen", {
        height: "500px",
        data: Object.values(resumenmetradoelectricas),
        layout: "fitColumns",
        dataTree: true,
        dataTreeStartExpanded: true,
     ...
     $('#actualizar_metrados').on('click', () => {
        const id_metrado_electricas = document.getElementById('id_melectricas').value;
        if (metradoElectricas && metradoElectricas.length > 0) {
            metradoElectricas = sortTreeData(database.modulos) || jsonTree;
        }

        // ✅ Solo reasignamos si jsonTree tiene datos nuevos
        if (jsonTree && jsonTree.length > 0) {
            metradoElectricas = jsonTree;  // Ahora sí se puede reasignar porque es `let`
        }


        //const datatable = metradoElectricas;
        const dataresumen = resumenMetradoElectricasTree;//Object.values(resumenmetradoelectricas);
        const dataToSend = {
            id: id_metrado_electricas,
            modulos: metradoElectricas,
            resumenel: dataresumen,
        };
    ...
    /*$('#actualizar_metrados').on('click', () => {
        ...
        console.log("datos de resumen", dataresumen);
        const comprimido = JSON.stringify(dataToSend)
        console.log("data comprimido", comprimido);
        $.ajax({
            url: '/update_metrados_electricas', // Laravel route
            method: 'POST',
            data: comprimido, // Send data as JSON
    ...
    document.getElementById('download-xlsx').addEventListener('click', function () {
        //Datos Excel
        const data = metradoElectricas;
        const dataresumen = (Object.values(resumenmetradoelectricas));
