...
class Gastos {
    ...
    loadData() {
        ...
        let costoDirectoFromServer = 0; // Variable para almacenar el costo directo
        // Llamada AJAX para obtener el costo directo
        $.ajax({
        ...
        $.ajax({
            ...
            success: (response) => {
                //console.log("Datos cargados de gastos fijos:", response);
                if (response && response.status === "success" && response.data && response.data.gastos_fijos) {
                    try {
                        ...
                        if (data && data.tables) {
                            ...
                            console.log("La propiedad 'tables' no se encontró en los datos de gastos fijos.");
                        }
                    } catch (error) {
                        ...
                        Swal.fire({
                            icon: "error",
                            title: "Error",
                            text: "Error al procesar los datos de gastos fijos recibidos del servidor."
                        });
                    }
                } else {
                    console.log("La respuesta del servidor de gastos fijos está vacía, tiene un estado de error o no contiene datos.");
                }

                // Después de obtener los totales de gastos fijos, cargar los gastos generales
            ...
            error: (xhr, status, error) => {
                ...
                Swal.fire({
                    icon: "error",
                    title: "Error",
                    text: "No se pudieron cargar los datos de gastos fijos."
                });
            }
        });
    }
